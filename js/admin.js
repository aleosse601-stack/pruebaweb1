document.addEventListener('DOMContentLoaded', async () => {
  ...
  function codigoUsuario(u) {
    const base = (u.id || u.usuario || u.nombre || '').toString();
  ...
  function buscarUsuarioPorCodigo(codigo) {
    ...
    return state.usuarios.find(u => {
      const codigos = [codigoUsuario(u), u.id, u.usuario, u.nombre].filter(Boolean).map(x => x.toString().toUpperCase());
  ...
  function renderStats() {
    ...
    document.title = `Admin · ${state.usuarios.length} usuarios`;
  }

  function renderAll() { renderEventos(); renderUsuarios(); renderComunicados(); renderFormularios(); renderMovimientos(); renderStats(); saveLocal(); }
  ...
  async function cobrarUsuario(codigo, importe, concepto) {
    ...
    if (!usuario) return resultado && (resultado.textContent = '❌ Usuario no encontrado', resultado.classList.add('error'));
    if (usuario.estado && usuario.estado !== 'activo') return resultado && (resultado.textContent = '❌ Usuario inactivo', resultado.classList.add('error'));
    if (!importe || importe <= 0) return resultado && (resultado.textContent = '❌ Importe no válido', resultado.classList.add('error'));
    if (Number(usuario.saldo || 0) < importe) return resultado && (resultado.textContent = `❌ Saldo insuficiente. Saldo actual: ${euro(usuario.saldo)}`, resultado.classList.add('error'));
    ...
    document.getElementById('cobroForm')?.reset(); renderAll();
  }
  ...
  document.getElementById('usuarioForm')?.addEventListener('submit', async e => {
    ...
    if (usingSupabase) {
      ...
      if (error) {
        console.error(error);
        alert('No se ha podido crear en Supabase. Se guardará temporalmente en este navegador.');
  ...
  document.getElementById('eventoForm')?.addEventListener('submit', async e => {
    ...
    if (usingSupabase) { const { data } = await supabaseClient.from('eventos').insert(nuevo).select().single(); state.eventos.push(data || nuevo); } else state.eventos.push(nuevo);
    e.target.reset(); renderAll();
  });
  ...
  document.getElementById('comunicadoForm')?.addEventListener('submit', async e => {
    ...
    if (usingSupabase) { const { data } = await supabaseClient.from('comunicados').insert(nuevo).select().single(); state.comunicados.unshift(data || nuevo); } else state.comunicados.unshift(nuevo);
    e.target.reset(); renderAll();
  });
  ...
  document.getElementById('formularioForm')?.addEventListener('submit', async e => {
    ...
    if (usingSupabase) { const { data } = await supabaseClient.from('formularios').insert(nuevo).select().single(); state.formularios.unshift(data || nuevo); } else state.formularios.unshift(nuevo);
    e.target.reset(); renderAll();
  });

  document.getElementById('cobroForm')?.addEventListener('submit', async e => { e.preventDefault(); await cobrarUsuario(cobroCodigo.value, Number(cobroImporte.value), cobroConcepto.value); });

  document.getElementById('startScanner')?.addEventListener('click', async () => {
    const status = document.getElementById('scannerStatus');
    if (typeof Html5Qrcode === 'undefined') return status && (status.textContent = 'El escáner no se ha cargado. Usa el código manual.');
    try {
      qrScanner = new Html5Qrcode('qr-reader');
      await qrScanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: 250 }, decodedText => { cobroCodigo.value = decodedText; if (status) status.textContent = 'QR detectado ✔️'; });
    } catch { if (status) status.textContent = 'No se pudo abrir la cámara. Usa el código manual.'; }
  });
  document.getElementById('stopScanner')?.addEventListener('click', async () => { if (qrScanner) await qrScanner.stop().catch(() => {}); qrScanner = null; const status = document.getElementById('scannerStatus'); if (status) status.textContent = 'Escáner detenido'; });

  document.addEventListener('click', async e => {
    if (e.target.dataset.delEvento) { const idx = Number(e.target.dataset.delEvento); const id = e.target.dataset.id; if (usingSupabase && id) await supabaseClient.from('eventos').delete().eq('id', id); state.eventos.splice(idx, 1); renderAll(); }
    if (e.target.dataset.delFormulario) { const idx = Number(e.target.dataset.delFormulario); const id = e.target.dataset.id; if (usingSupabase && id) await supabaseClient.from('formularios').delete().eq('id', id); state.formularios.splice(idx, 1); renderAll(); }
    if (e.target.dataset.saveUsuario) {
      const idx = Number(e.target.dataset.saveUsuario); const usuario = state.usuarios[idx]; const saldoAnterior = Number(usuario.saldo || 0); const nuevoSaldo = Number(document.querySelector(`[data-saldo="${idx}"]`).value || 0); const nuevaClave = document.querySelector(`[data-password="${idx}"]`).value.trim();
      ...
      const diferencia = nuevoSaldo - saldoAnterior; if (diferencia !== 0) await insertarMovimiento({ usuario: usuario.nombre, concepto: 'Ajuste manual admin', importe: diferencia }); renderAll();
    }
    if (e.target.dataset.deleteUsuario) { const idx = Number(e.target.dataset.deleteUsuario); const id = e.target.dataset.id; const usuario = state.usuarios[idx]; if (!confirm(`¿Eliminar a ${usuario.nombre || 'este usuario'}?`)) return; if (usingSupabase && id) { const { error } = await supabaseClient.from('usuarios').delete().eq('id', id); if (error) return alert('No se ha podido eliminar el usuario.'); } state.usuarios.splice(idx, 1); renderAll(); }
    if (e.target.dataset.addSaldo) { const idx = Number(e.target.dataset.addSaldo); const usuario = state.usuarios[idx]; const nuevoSaldo = Number(usuario.saldo || 0) + 5; await actualizarUsuario(usuario, { saldo: nuevoSaldo }); await insertarMovimiento({ usuario: usuario.nombre, concepto: 'Recarga admin', importe: 5 }); renderAll(); }
  });

  await loadSupabase(); renderAll();
});
